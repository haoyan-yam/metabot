...
| 14 | large-file-chunk-download | N | 超 100MB 附件（单次 GET 报 234037）自动转 HTTP Range 分片下载；下载失败不再静默——prompt 里写明文件名与原因；群聊媒体缓存 TTL 5→30 分钟、过期丢弃打 WARN（该缓存已被补丁 18 的接口拉取取代） |
...
- **补丁 18 让私聊也必须 @，并按接口拉本轮上下文**：打上后私聊/两人群不 @ 就不回（静默），这是本部署刻意的选择——飞书私聊里发图片/文件会立刻提交给 bot 开始分析，而团队习惯是「先把文件、链接、要求发齐，最后 @ 一句处理」；@ 时桥接用 `im.message.list` 拉这个人本轮的消息一起带上，需要应用有读取单聊/群消息的权限（本部署 16 个应用现成可用，缺权限时优雅降级）。想保留上游「私聊直接回」行为的，不要打 18。补丁 N 里 30 分钟媒体缓存的说明已随之失效，N 只剩分片下载与下载失败提示。
